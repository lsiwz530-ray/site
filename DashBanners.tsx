...
app.post("/api/admin/set-role", requireAdmin, async (req, res) => {
  ...
  if (!["admin"].includes(role)) return res.status(400).json({ error: "unknown role" });
  try {
    if (grant) {
      await pool.query(
      ...
      const { rows: c } = await pool.query(`SELECT count(*)::int AS n FROM user_roles WHERE role='admin'`);
      if (c[0].n <= 1) return res.status(400).json({ error: "لا يمكن إزالة آخر أدمن." });
